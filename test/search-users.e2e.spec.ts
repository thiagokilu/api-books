...
describe("Search Users (E2E)", () => {
  ...
  const testEmail = `e2e-search-${runId}@email.com`;
  const testUsername = `e2e-search-user-${runId}`;
  ...
  it("should search users by username successfully and return 200", async () => {
    const response = await app.inject({
      method: "GET",
      url: `/search-users/${testUsername}`,
      headers: {
